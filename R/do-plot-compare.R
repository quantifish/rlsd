...
do_plot_compare <- function(object_list, object_names, single_stock, figure_dir = "compare_figure/")
{
	...
	if(single_stock==FALSE){
		...
		plot_curves(risk_summary=risk_summary, msy=msy$msy, figure_dir = figure_dir)
		plot_timeseries(model_info=msy, object_list=object_list, object_names=object_names, figure_dir = figure_dir)
