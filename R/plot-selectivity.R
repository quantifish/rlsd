...
plot_selectivity <- function(object,
  ...
  p <- ggplot(data = sel2, aes(x = .data$Size, y = .data$Selectivity, col = .data$Year, fill = .data$Year)) +
    stat_summary(data = sel2, aes(x = .data$Size, y = .data$Selectivity, col = .data$Year), fun.min = function(x) quantile(x, 0.025), fun.max = function(x) quantile(x, 0.975), geom = "ribbon", alpha = 0.25, colour = NA) +
  ...
  ggsave(filename = paste0(figure_dir, "selectivity.png"), plot = p, width = 8, height = 8)
  ...
  ggsave(filename = paste0(figure_dir, "selectivity_ridges.png"), plot = q)
  ...
  p <- ggplot(data = vs2, aes(x = .data$Size, y = .data$SelVuln, col = .data$Season, fill = .data$Season)) +
    ...
    scale_y_continuous(expand = c(0, 0)) +
    coord_cartesian(ylim = c(0, 1)) +
    labs(x = xlab, y = "Selectivity * Vulnerability") +
    theme_lsd()
  ...
  ggsave(filename = paste0(figure_dir, "selectivity_vulnerability.png"), plot = p, width = 12, height = 9)
}
