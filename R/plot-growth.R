...
#'
#' Plot the 6-monthly growth increment
#'
...
#' @export
#'
plot_growth_increment <- function(object, xlab = "Size (mm)", ylab = "Increment (mm)",
  ...
  gi <- reshape2::melt(gi) %>%
    spread(Type, value) %>%
    mutate(Lo = Increment - SD, Hi = Increment + SD) %>%
    inner_join(w, by = "Morph") %>%
    distinct(Iteration, Increment, Region, .keep_all = TRUE)
...
#' @export
#'
plot_tag_residuals <- function(object, ylab = "Standardised residual",
                               figure_dir = "figure/", ylim = c(-5, 5)) {
  data <- object@data
  map <- object@map
  mcmc <- object@mcmc
  ...
  #dimnames(w) <- list("Region" = object@regions, "Sex" = sex, "Year" = pyears)
  #w <- reshape2::melt(w, value.name = "Morph") %>%
  #    distinct(Region, Sex, Morph, .keep_all = TRUE)
  ...
  resid <- reshape2::melt(resid) %>%
    inner_join(morph, by = "I")
  #inner_join(w, by = "I") %>%
  #distinct(Iteration, I, Region, .keep_all = TRUE)
...
plot_growth_matrix <- function(object,
  ...
  tag <- data.frame(Liberty = data$cov_grow_liberty_g, Sex = data$cov_grow_morph_g, capture = data$data_grow_size_capture_g, recapture = data$data_grow_size_recapture_g) %>%
    mutate(Size1 = bins[cut(capture, breaks = bins - 1, labels = FALSE)], Size2 = bins[cut(recapture, breaks = bins - 1, labels = FALSE)]) %>%
    filter(Sex == 1, Liberty > 0.5, Liberty < 1.5) %>%
    select(Size1, Size2)
  ttag <- table(tag)
  ...
  G <- reshape2::melt(G) %>%
    mutate(Size1 = as.factor(Size1), File = as.factor(File))# %>%
  #filter(Season == "AW", File == 1, Size1 %in% c(31,51,91))

  p <- ggplot(data = filter(G, Season == "AW", value > 0), aes(x = Size2, y = value, colour = Size1, fill = Size1, group = Size1)) +
  ...
  p <- ggplot(data = filter(G, Season == "AW", value > 0), aes(x = Size2, y = value, colour = File, fill = File)) +
  ...
  GG <- filter(G, File == 1, Size1 %in% ii[(length(ii)-5):length(ii)], Season == "AW", value > 0)
  ...
  GG <- filter(G, File == 1, Season == "AW", value > 0)
