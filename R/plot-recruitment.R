...
plot_recruitment_deviations <- function(object,
  ...
  if (!is.null(rdevs2)) {
    p <- p +
      stat_summary(data = rdevs2, aes(x = Year, y = value), fun.min = function(x) quantile(x, 0.025), fun.max = function(x) quantile(x, 0.975), geom = "ribbon", alpha = 0.25, colour = NA) +
  ...
  ggsave(filename = paste0(figure_dir, "recruitment_deviations.png"), plot = p)
}
...
plot_recruitment <- function(object,
  ...
  if (!is.null(recruits2)) {
    p <- p +
      stat_summary(data = R02 %>% filter(Year %in% rdev_years), aes(x = Year, y = value/1e+6), fun.min = function(x) quantile(x, 0.025), fun.max = function(x) quantile(x, 0.975), geom = "ribbon", alpha = 0.25, colour = NA, fill = "green") +
      stat_summary(data = R02 %>% filter(Year %in% rdev_years), aes(x = Year, y = value/1e+6), fun.y = function(x) quantile(x, 0.5), geom = "line", lwd = 1, colour = "green") +
      stat_summary(data = recruits2 %>% filter(Year %in% rdev_years), aes(x = Year, y = value/1e+6), fun.min = function(x) quantile(x, 0.025), fun.max = function(x) quantile(x, 0.975), geom = "ribbon", alpha = 0.25, colour = NA) +
      stat_summary(data = recruits2 %>% filter(Year %in% rdev_years), aes(x = Year, y = value/1e+6), fun.y = function(x) quantile(x, 0.5), geom = "line", lwd = 1)
  }

  if (!is.null(recruits1)) {
    p <- p +
      geom_line(data = R01 %>% filter(Year %in% rdev_years), aes(x = Year, y = value/1e+6), linetype = 2, colour = "green") +
  ...
  if (data$n_area > 1) p <- p + facet_wrap(~ Region)
  ggsave(filename = paste0(figure_dir, "recruitment.png"), plot = p)
  ...
  if (!is.null(recruits2)) {
    p <- p +
      stat_summary(data = R02, aes(x = Year, y = value/1e+6), fun.min = function(x) quantile(x, 0.025), fun.max = function(x) quantile(x, 0.975), geom = "ribbon", alpha = 0.25, colour = NA, fill = "green") +
      stat_summary(data = R02, aes(x = Year, y = value/1e+6), fun.y = function(x) quantile(x, 0.5), geom = "line", lwd = 1, colour = "green") +
      stat_summary(data = recruits2, aes(x = Year, y = value/1e+6), fun.min = function(x) quantile(x, 0.05), fun.max = function(x) quantile(x, 0.95), geom = "ribbon", alpha = 0.25, colour = NA) +
      stat_summary(data = recruits2, aes(x = Year, y = value/1e+6), fun.min = function(x) quantile(x, 0.25), fun.max = function(x) quantile(x, 0.75), geom = "ribbon", alpha = 0.5, colour = NA) +
      stat_summary(data = recruits2, aes(x = Year, y = value/1e+6), fun.y = function(x) quantile(x, 0.5), geom = "line", lwd = 1)
  }

  if (!is.null(recruits1)) {
    p <- p +
      geom_line(data = R01, aes(x = Year, y = value/1e+6), linetype = 2, colour = "green") +
  ...
  ggsave(filename = paste0(figure_dir, "recruitment_v1.png"), plot = p1)

  p2 <- p + geom_vline(aes(xintercept = data$last_yr), linetype = "dashed")
  ggsave(filename = paste0(figure_dir, "recruitment_v2.png"), plot = p2)
...
plot_recruitment_size <- function(object, xlab = "Size at recruitment (mm)", ylab = "Proportion", figure_dir = "figure/") {
  ...
  mcmc$recruitment_size_dist_l <- melt(mcmc$recruitment_size_dist_l) %>%
    filter(Iteration == 1)
  ...
  ggsave(filename = paste0(figure_dir, "recruitment_size.png"), plot = p)
}
