                       ...
                       figure_dir = "figure/")
{
    ...
    pcatch_sum <- dplyr::group_by(pcatch, Iteration, Region, Year, Type, Data) %>%
        dplyr::summarise(Catch = sum(Catch)) %>%
    ...
    # Catch residuals
    rsl <- mcmc$resid_catch_sl_ryt
    dimnames(rsl) <- list("Iteration" = 1:n_iter, "Region" = regions,
                          "Year" = pyears, "Season" = seasons)
    rsl <- reshape2::melt(rsl, value.name = "Catch") %>%
        dplyr::mutate(Type = "SL", Data = "Residual")
    rnsl <- mcmc$resid_catch_nsl_ryt
    dimnames(rnsl) <- list("Iteration" = 1:n_iter, "Region" = regions,
                           "Year" = pyears, "Season" = seasons)
