...
plot_sex_ratio <- function(object, scales = "free", xlab = "Fishing year", ylab = "Proportion", figure_dir = "figure/")
{
    ...
    if (length(mcmc) > 0) {
        p <- p + stat_summary(data = psexr2, aes(x = Year, y = value), fun.ymin = function(x) quantile(x, 0.05), fun.ymax = function(x) quantile(x, 0.95), geom = "ribbon", alpha = 0.25, colour = NA) +
          stat_summary(data = psexr2, aes(x = Year, y = value), fun.ymin = function(x) quantile(x, 0.25), fun.ymax = function(x) quantile(x, 0.75), geom = "ribbon", alpha = 0.5, colour = NA) +
          stat_summary(data = psexr2, aes(x = Year, y = value), fun.y = function(x) quantile(x, 0.5), geom = "line", lwd = 1)
    }
