...
plot_cpue <- function(object,
  ...
  # Observed CPUE
  ocpue <- data.frame(Iteration = NA, CPUE = data$data_cpue_i, Data = "Observed",
                      Type = "CPUE", Region = paste0("Region: ", data$data_cpue_area_i),
                      Year = data$data_cpue_year_i, Season = seasons[data$data_cpue_season_i],
  ...
  ocpue <- ocpue %>%
    mutate(CPUE_name = case_when(
      Year < 1979 & CPUE_type == 1 ~ "CR",
  ...
  # Predicted CPUE
  scpue <- mcmc$sigma_cpue_i
  dimnames(scpue) <- list("Iteration" = 1:n_iter, "I" = 1:data$n_cpue)
  scpue <- melt(scpue, value.name = "SD")

  pcpue <- mcmc$pred_cpue_i
  dimnames(pcpue) <- list("Iteration" = 1:n_iter, "I" = 1:data$n_cpue)
  pcpue <- melt(pcpue, value.name = "CPUE") %>%
    full_join(scpue, by = join_by(Iteration, I)) %>%
    select(Iteration, CPUE, SD) %>%
    mutate(Data = "Expected", Type = "CPUE",
           Region = rep(data$data_cpue_area_i, each = n_iter),
           Year = rep(data$data_cpue_year_i, each = n_iter),
           Season = seasons[rep(data$data_cpue_season_i, each = n_iter)]) %>%
  ...
  pcpue <- pcpue %>%
    ...
    mutate(PP = rlnorm(n = n(), meanlog = log(CPUE), sdlog = SD)) %>%
  ...
  rcpue <- melt(rcpue, value.name = "CPUE") %>%
    ...
    mutate(Data = "Residual", Type = "CPUE",
           Region = rep(data$data_cpue_area_i, each = n_iter),
           Year = rep(data$data_cpue_year_i, each = n_iter),
           Season = seasons[rep(data$data_cpue_season_i, each = n_iter)]) %>%
    mutate(Region = paste0("Region: ", Region))
  ...
  if (!is.null(pcpue)) {
    p <- p +
      stat_summary(data = pcpue, aes(x = Year, y = PP), fun.min = function(x) quantile(x, 0.025), fun.max = function(x) quantile(x, 0.975), geom = "ribbon", alpha = 0.25, colour = NA) +
      stat_summary(data = pcpue, aes(x = Year, y = CPUE), fun.min = function(x) quantile(x, 0.025), fun.max = function(x) quantile(x, 0.975), geom = "ribbon", alpha = 0.25, colour = NA) +
  ...
  if (data$n_area > 1) {
    ...
    ggsave(filename = paste0(figure_dir, "cpue.png"), plot = p, height = 10, width = 15)
  } else {
    p <- p + facet_wrap(Season ~ CPUE_name, scales = "free", nrow = length(unique(ocpue$Season)))
    ggsave(filename = paste0(figure_dir, "cpue.png"), plot = p, height = 9, width = 12)
  }
  ...
  if (!is.null(rel_pcpue)) {
    p <- p + stat_summary(data = rel_pcpue, aes(x = Year, y = RelCPUE), fun.min = function(x) quantile(x, 0.05), fun.max = function(x) quantile(x, 0.95), geom = "ribbon", alpha = 0.25, colour = NA) +
      stat_summary(data = rel_pcpue, aes(x = Year, y = RelCPUE), fun = function(x) quantile(x, 0.25), fun.max = function(x) quantile(x, 0.75), geom = "ribbon", alpha = 0.5, colour = NA) +
  ...
  if (data$n_area > 1) {
    ...
    ggsave(filename = paste0(figure_dir, "cpue_rel.png"), p, height = 10, width = 15)
  } else {
    p <- p + facet_grid(Season ~ CPUE_name, scales = "free")
    ggsave(filename = paste0(figure_dir, "cpue_rel.png"), p, height = 9, width = 12)
  }

  # Separate by series
  # CR
  ocr_yrs <- ocpue %>% filter(Year < 1979)
  if (nrow(ocr_yrs) > 0) {
    pcr_yrs <- pcpue %>% filter(Year < 1979)
    # p1cr_yrs <- pcpue1 %>%  filter(Year < 1979)
    ...
    if (!is.null(pcpue)) {
      p <- p +
        stat_summary(data = pcr_yrs, aes(x = Year, y = PP), fun.min = function(x) quantile(x, 0.025), fun.max = function(x) quantile(x, 0.975), geom = "ribbon", alpha = 0.25, colour = NA) +
        stat_summary(data = pcr_yrs, aes(x = Year, y = CPUE), fun.min = function(x) quantile(x, 0.025), fun.max = function(x) quantile(x, 0.975), geom = "ribbon", alpha = 0.5, colour = NA) +
    ...
    if (length(unique(ocr_yrs$Region)) > 1) {
      ...
      ggsave(filename = paste0(figure_dir, "cpue_CR.png"), plot = p, width = 15, height = 10)
    } else {
      p <- p + facet_wrap(~ Season, scales = "free_y", ncol = 1)
      ggsave(filename = paste0(figure_dir, "cpue_CR.png"), plot = p, height = 9, width = 12)
    }
  }

  # FSU
  ocpue_fsu <- ocpue %>% filter(CPUE_name == "FSU")
  pcpue_fsu <- pcpue %>% filter(CPUE_name == "FSU")
  if (nrow(ocpue_fsu) > 0) {
    ...
    if (!is.null(pcpue_fsu)) {
      p <- p +
        stat_summary(data = pcpue_fsu, aes(x = Year, y = PP), fun.min = function(x) quantile(x, 0.025), fun.max = function(x) quantile(x, 0.975), geom = "ribbon", alpha = 0.25, colour = NA) +
        stat_summary(data = pcpue_fsu, aes(x = Year, y = CPUE), fun.min = function(x) quantile(x, 0.025), fun.max = function(x) quantile(x, 0.975), geom = "ribbon", alpha = 0.5, colour = NA) +
    ...
    if (length(unique(ocpue_fsu$Region)) > 1) {
      p <- p + facet_wrap(Region ~ Season, scales = "free", ncol = data$n_area)
      ggsave(filename = paste0(figure_dir, "cpue_FSU.png"), plot = p, width = 15, height = 10)
    } else {
      p <- p + facet_wrap(~ Season, scales = "free_y", ncol = data$n_area)
      ggsave(filename = paste0(figure_dir, "cpue_FSU.png"), plot = p, height = 9, width = 12)
    }
  }

  # CELR
  ocpue_celr <- ocpue %>% filter(CPUE_name == "CELR")
  pcpue_celr <- pcpue %>% filter(CPUE_name == "CELR")
  if (nrow(ocpue_celr) > 0) {
    ...
    if (!is.null(pcpue_celr)) {
      p <- p +
        stat_summary(data = pcpue_celr, aes(x = Year, y = PP), fun.min = function(x) quantile(x, 0.025), fun.max = function(x) quantile(x, 0.975), geom = "ribbon", alpha = 0.25, colour = NA) +
        stat_summary(data = pcpue_celr, aes(x = Year, y = CPUE), fun.min = function(x) quantile(x, 0.025), fun.max = function(x) quantile(x, 0.975), geom = "ribbon", alpha = 0.5, colour = NA) +
    ...
    if (length(unique(ocpue_celr$Region)) > 1) {
      p <- p + facet_wrap(Region~Season, scales = "free", ncol = data$n_area)
      ggsave(filename = paste0(figure_dir, "cpue_CELR.png"), plot = p, height = 10, width = 15)
    } else {
      p <- p + facet_wrap(~Season, scales = "free_y", ncol = data$n_area)
      ggsave(filename = paste0(figure_dir, "cpue_CELR.png"), plot = p, height = 9, width = 12)
    }
  }
  ...
  if (nrow(ocpue_lb) > 0) {
    ...
    if (!is.null(pcpue_lb)) {
      p <- p +
        stat_summary(data = pcpue_lb, aes(x = Year, y = PP), fun.min = function(x) quantile(x, 0.025), fun.max = function(x) quantile(x, 0.975), geom = "ribbon", alpha = 0.25, colour = NA) +
        stat_summary(data = pcpue_lb, aes(x = Year, y = CPUE), fun.min = function(x) quantile(x, 0.025), fun.max = function(x) quantile(x, 0.975), geom = "ribbon", alpha = 0.5, colour = NA) +
    ...
    if (length(unique(ocpue_lb$Region)) > 1) {
      ...
      ggsave(paste0(figure_dir, "cpue_Logbook.png"), plot = p, height = 14, width = 15)
    } else {
      p <- p + facet_wrap(qtype~Season, scales = "free_y", ncol = data$n_area)
      ggsave(filename = paste0(figure_dir, "cpue_Logbook.png"), plot = p, height = 11, width = 12)
    }
  }
  ...
  if (nrow(ocpue_lb) > 0) {
    ...
    if (length(unique(ocpue_lb$Region)) > 1) {
      p <- p + facet_wrap(qtype+Region~Season, scales = "free", ncol = data$n_area)
      ggsave(paste0(figure_dir, "cpue_CS.png"), plot = p, height = 14, width = 15)
    } else {
      p <- p + facet_wrap(qtype~Season, scales = "free_y", ncol = data$n_area)
      ggsave(filename = paste0(figure_dir, "cpue_CS.png"), plot = p, height = 11, width = 12)
    }
  }
  ...
  ggsave(filename = paste0(figure_dir, "cpue_resid.png"), plot = p, height = 10, width = 12)

  # Separate by series
  # CR
  rcr_yrs <- rcpue %>% filter(Year < 1979)
  if (nrow(rcr_yrs) > 0) {
    p <- ggplot(rcr_yrs) +
      geom_hline(yintercept = 0, alpha = 0.2) +
      xlab(xlab) + ylab("Standardised residual") +
    ...
    ggsave(filename = paste0(figure_dir, "cpue_resid_CR.png"), p, height = 9, width = 12)
  }

  # FSU
  rcpue_fsu <- rcpue %>% filter(CPUE_name == "FSU")
  if (nrow(rcpue_fsu) > 0) {
    p <- ggplot(rcpue_fsu) +
      geom_hline(yintercept = 0, alpha = 0.2) +
    ...
    ggsave(filename = paste0(figure_dir, "cpue_resid_FSU.png"), p, height = 9, width = 12)
  }

  # CELR
  rcpue_celr <- rcpue %>% filter(CPUE_name == "CELR")
  if (nrow(rcpue_celr) > 0) {
    ...
    ggsave(filename = paste0(figure_dir, "cpue_resid_CELR.png"), p, height = 9, width = 12)
  }

  # Logbook
  rcpue_lb <- rcpue %>% filter(CPUE_name == "Logbook")
  if (nrow(rcpue_lb) > 0) {
    p <- ggplot(rcpue_lb) +
      geom_hline(yintercept = 0, alpha = 0.2) +
      # expand_limits(y = 0) +
    ...
    ggsave(filename = paste0(figure_dir, "cpue_resid_Logbook.png"), p, height = 14, width = 12)
  }

  # CS
  rcpue_lb <- rcpue %>% filter(CPUE_name == "CS")
  if (nrow(rcpue_lb) > 0) {
    p <- ggplot(rcpue_lb) +
      geom_hline(yintercept = 0, alpha = 0.2) +
      # expand_limits(y = 0) +
    ...
    ggsave(filename = paste0(figure_dir, "cpue_resid_CS.png"), plot = p, height = 14, width = 12)
  }
}
...
plot_offset_cpue_lm <- function(object, figure_dir = "figure/") {
  ...
  ggsave(filename = paste0(figure_dir, "lm_offset_cpue.png"), plot = p)
}
...
plot_aw_cpue_lm <- function(object, figure_dir = "figure/") {
  ...
  ggsave(filename = paste0(figure_dir, plot = "lm_aw_cpue.png"), p)
...
plot_offset_cpue <- function(object,
  ...
  obs_offset <- melt(obs_offset) %>% mutate(Iteration = NA, Data = "Observed")
  ...
  if (!is.null(mcmc_offset)) {
    p <- p + stat_summary(data = mcmc_offset, aes(x = Year, y = value), fun.min = function(x) quantile(x, 0.05), fun.max = function(x) quantile(x, 0.95), geom = "ribbon", alpha = 0.25, colour = NA) +
      stat_summary(data = mcmc_offset, aes(x = Year, y = value), fun.min = function(x) quantile(x, 0.25), fun.max = function(x) quantile(x, 0.75), geom = "ribbon", alpha = 0.5, colour = NA) +
  ...
  ggsave(filename = paste0(figure_dir, "cpue_offset.png"), plot = p, width = 12)
}
